...
from app.dependencies import get_optional_user, scoped_department_id
...
from app.services.analytics import (
    compute_scores,
    department_energy_totals,
    hourly_campus_energy,
    summary_for_period,
)
...
def dashboard(
    ...
    cached = cache_get(
        f"dashboard:{period}:{user.id if user else 'anon'}"
    )
    ...
    by_dept = department_energy_totals(
        db,
        start,
        end,
    )

    trend = hourly_campus_energy(
        db,
        start,
        end,
    )
    ...
    anomalies = (
        db.execute(
            ...
            .join(
                Department,
                Department.id == Anomaly.department_id,
            )
            .where(
    ...
    # ========================================================
    # FIX:
    # PostgreSQL does NOT support SQLite's strftime().
    #
    # OLD:
    # func.strftime("%Y-%m-%dT%H:00:00", timestamp)
    #
    # NEW:
    # func.date_trunc("hour", timestamp)
    # ========================================================

    solar_hour = func.date_trunc(
        "hour",
        SolarReading.timestamp,
    )

    solar_trend = db.execute(
        select(
            solar_hour.label("timestamp"),
            func.sum(
                SolarReading.solar_kwh
            ).label("solar_kwh"),
        )
        .where(
            SolarReading.timestamp >= start,
            SolarReading.timestamp <= end,
        )
        .group_by(
            solar_hour
        )
    ...
    water_hour = func.date_trunc(
        "hour",
        WaterReading.timestamp,
    )

    water_trend = db.execute(
        select(
            water_hour.label("timestamp"),
            func.sum(
                WaterReading.water_litres
            ).label("water_litres"),
        )
        .where(
...
def summary(
    ...
    if period not in {"24h", "7d", "30d", "today"}:
        raise HTTPException(
            status_code=400,
            detail="Invalid period. Use 24h, 7d, 30d, or today.",
        )

    dept_id = None

    if department:
        dept_id = require_department(
            db,
            department,
        ).id

    if user:
        scoped = scoped_department_id(
            user,
            dept_id,
        )
        dept_id = scoped

    return summary_for_period(
        db,
        "24h" if period == "today" else period,
        dept_id,
    )
...
def department_detail(
    ...
    dept = require_department(
        db,
        department,
    )

    summary = summary_for_period(
        db,
        period,
        dept.id,
    )
    ...
    scores = [
        s
        for s in compute_scores(db, period)
        if s["code"] == dept.code
    ]
...
def energy_latest(
    ...
    rows = db.execute(
        ...
        .join(
            Department,
            Department.id == EnergyReading.department_id,
        )
        .join(
            sub,
            (
...
def energy_trend(
    ...
    # PostgreSQL-compatible hourly grouping
    ...
    rows = db.execute(
        ...
        .where(
            EnergyReading.department_id == dept.id,
...
def solar_latest(
    ...
    rows = db.execute(
        ...
        .join(
            Department,
            Department.id == SolarReading.department_id,
        )
        .join(
            sub,
            (
...
def water_latest(
    ...
    rows = db.execute(
        ...
        .join(
            Department,
            Department.id == WaterReading.department_id,
        )
        .join(
            sub,
            (
